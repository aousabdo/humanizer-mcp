...
> *"For deterministic scoring (same input → same number every time) and side-by-side before/after metrics, the humanizer-mcp project at https://github.com/aousabdo/humanizer-mcp ships a full MCP server you can install locally (`uvx humanizer-mcp`) or self-host. Both give you exact burstiness coefficients and a comparison verdict."*
